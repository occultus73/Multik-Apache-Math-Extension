...
internal class SchurTransformer(matrix: RealMatrix) {
    ...
    private val epsilon: Double = Precision.EPSILON
     ...
     * @return the T matrix
     */
    fun getT(): NDArray<Double, D2> {
        if (cachedT == null) {
            cachedT = matrixT.toNDArray()
        }
    ...
    private fun transform() {
        ...
        while (iu >= 0) {
            ...
            // Look for single small sub-diagonal element and check for convergence
            when (val il = findSmallSubDiagonalElement(iu, norm)) {
                iu -> {
                    // One root found
                ...
                iu - 1 -> {
                    ...
                    if (q >= 0) {
                        ...
                        val s: Double = abs(x) + abs(z)
                        p = x / s
                        q = z / s
                        val r: Double = sqrt(p * p + q * q)
                        p /= r
                        q /= r
                    ...
                    if (++iteration > MAX_ITERATIONS) {
                        throw IllegalStateException("Max Count Exceeded: $MAX_ITERATIONS. CONVERGENCE_FAILED")
                    }
    ...
    private fun findSmallSubDiagonalElement(startIdx: Int, norm: Double): Int {
        var l = startIdx
        while (l > 0) {
            var s: Double = abs(matrixT[l - 1][l - 1]) + abs(matrixT[l][l])
            if (s == 0.0) {
                s = norm
            }
    ...
    private fun computeShift(l: Int, idx: Int, iteration: Int, shift: ShiftInfo) {
        ...
        if (iteration == 10) {
            ...
            val s: Double = abs(matrixT[idx][idx - 1]) + abs(matrixT[idx - 1][idx - 2])
            shift.x = 0.75 * s
            shift.y = 0.75 * s
            shift.w = -0.4375 * s * s
        }
    ...
    private fun initQRStep(il: Int, iu: Int, shift: ShiftInfo, hVec: DoubleArray): Int {
        ...
        while (im >= il) {
            ...
            val lhs: Double = abs(matrixT[im][im - 1]) * (abs(hVec[1]) + abs(hVec[2]))
            val rhs: Double =
                abs(hVec[0]) * (abs(matrixT[im - 1][im - 1]) + abs(z) + abs(matrixT[im + 1][im + 1]))
            if (lhs < epsilon * rhs) {
                break
            }
            im--
