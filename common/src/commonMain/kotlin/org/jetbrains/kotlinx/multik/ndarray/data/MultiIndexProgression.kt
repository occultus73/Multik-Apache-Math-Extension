...
class MultiIndexProgression(val first: IntArray, val last: IntArray, val step: Int = 1) {
    ...
    val reverse: MultiIndexProgression by lazy {
        last downTo first
    }
    ...
    override fun equals(other: Any?): Boolean =
        other is MultiIndexProgression && (first.contentEquals(other.first) && last.contentEquals(
            other.last
        ))
    ...
    override fun toString(): String {
        ...
        }..${last.joinToString(prefix = "(", postfix = ")")}"
    }
}

internal class MultiIndexIterator(first: IntArray, last: IntArray, private val step: Int) :
    Iterator<IntArray> {
    ...
    private val next =
        if (hasNext) first.copyOf().apply { this[lastIndex] -= step } else finalElement
