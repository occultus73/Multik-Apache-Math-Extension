 ...
 */
interface ImmutableMemoryView<T : Number> : Iterable<T> {
    val data: Any
 ...
 */
sealed class MemoryView<T : Number> : ImmutableMemoryView<T> {
    ...
    operator fun plusAssign(other: MemoryView<T>): Unit {
        when {
    ...
    open operator fun plusAssign(other: T): Unit {
        when {
    ...
    operator fun minusAssign(other: MemoryView<T>): Unit {
        when {
            this is MemoryViewFloatArray && other is MemoryViewFloatArray -> this -= other
            this is MemoryViewIntArray && other is MemoryViewIntArray -> this -= other
            this is MemoryViewDoubleArray && other is MemoryViewDoubleArray -> this -= other
    ...
    open operator fun minusAssign(other: T): Unit {
        when {
            this is MemoryViewFloatArray && other is Float -> this -= other
            this is MemoryViewIntArray && other is Int -> this -= other
            this is MemoryViewDoubleArray && other is Double -> this -= other
    ...
    operator fun timesAssign(other: MemoryView<T>): Unit {
        when {
            this is MemoryViewFloatArray && other is MemoryViewFloatArray -> this *= other
            this is MemoryViewIntArray && other is MemoryViewIntArray -> this *= other
            this is MemoryViewDoubleArray && other is MemoryViewDoubleArray -> this *= other
    ...
    open operator fun timesAssign(other: T): Unit {
        when {
            this is MemoryViewFloatArray && other is Float -> this *= other
            this is MemoryViewIntArray && other is Int -> this *= other
            this is MemoryViewDoubleArray && other is Double -> this *= other
    ...
    operator fun divAssign(other: MemoryView<T>): Unit {
        when {
            this is MemoryViewFloatArray && other is MemoryViewFloatArray -> this /= other
            this is MemoryViewIntArray && other is MemoryViewIntArray -> this /= other
            this is MemoryViewDoubleArray && other is MemoryViewDoubleArray -> this /= other
    ...
    open operator fun divAssign(other: T): Unit {
        when {
            this is MemoryViewFloatArray && other is Float -> this /= other
            this is MemoryViewIntArray && other is Int -> this /= other
            this is MemoryViewDoubleArray && other is Double -> this /= other
...
class MemoryViewByteArray(override val data: ByteArray) : MemoryView<Byte>() {
    ...
    operator fun plusAssign(other: MemoryViewByteArray): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] + other.data[i]).toByte()
        }
    }

    override operator fun plusAssign(other: Byte): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] + other).toByte()
        }
    }

    operator fun minusAssign(other: MemoryViewByteArray): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] - other.data[i]).toByte()
        }
    }

    override operator fun minusAssign(other: Byte): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] - other).toByte()
        }
    }

    operator fun timesAssign(other: MemoryViewByteArray): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] * other.data[i]).toByte()
        }
    }

    override operator fun timesAssign(other: Byte): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] * other).toByte()
        }
    }

    operator fun divAssign(other: MemoryViewByteArray): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] / other.data[i]).toByte()
        }
    }

    override operator fun divAssign(other: Byte): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] / other).toByte()
        }
    }
...
class MemoryViewShortArray(override val data: ShortArray) : MemoryView<Short>() {
    ...
    operator fun plusAssign(other: MemoryViewShortArray): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] + other.data[i]).toShort()
        }
    }

    override operator fun plusAssign(other: Short): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] + other).toShort()
        }
    }

    operator fun minusAssign(other: MemoryViewShortArray): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] - other.data[i]).toShort()
        }
    }

    override operator fun minusAssign(other: Short): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] - other).toShort()
        }
    }

    operator fun timesAssign(other: MemoryViewShortArray): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] * other.data[i]).toShort()
        }
    }

    override operator fun timesAssign(other: Short): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] * other).toShort()
        }
    }

    operator fun divAssign(other: MemoryViewShortArray): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] / other.data[i]).toShort()
        }
    }

    override operator fun divAssign(other: Short): Unit {
        for (i in this.indices) {
            this.data[i] = (this.data[i] / other).toShort()
        }
    }
...
class MemoryViewIntArray(override val data: IntArray) : MemoryView<Int>() {
    ...
    operator fun plusAssign(other: MemoryViewIntArray): Unit {
        for (i in this.indices) {
            this.data[i] += other.data[i]
        }
    }

    override operator fun plusAssign(other: Int): Unit {
        for (i in this.indices) {
            this.data[i] += other
        }
    }

    operator fun minusAssign(other: MemoryViewIntArray): Unit {
        for (i in this.indices) {
            this.data[i] -= other.data[i]
        }
    }

    override operator fun minusAssign(other: Int): Unit {
        for (i in this.indices) {
            this.data[i] -= other
        }
    }

    operator fun timesAssign(other: MemoryViewIntArray): Unit {
        for (i in this.indices) {
            this.data[i] *= other.data[i]
        }
    }

    override operator fun timesAssign(other: Int): Unit {
        for (i in this.indices) {
            this.data[i] *= other
        }
    }

    operator fun divAssign(other: MemoryViewIntArray): Unit {
        for (i in this.indices) {
            this.data[i] /= other.data[i]
        }
    }

    override operator fun divAssign(other: Int): Unit {
        for (i in this.indices) {
            this.data[i] += other
        }
    }
...
class MemoryViewLongArray(override val data: LongArray) : MemoryView<Long>() {
    ...
    operator fun plusAssign(other: MemoryViewLongArray): Unit {
        for (i in this.indices) {
            this.data[i] += other.data[i]
        }
    }

    override operator fun plusAssign(other: Long): Unit {
        for (i in this.indices) {
            this.data[i] += other
        }
    }

    operator fun minusAssign(other: MemoryViewLongArray): Unit {
        for (i in this.indices) {
            this.data[i] -= other.data[i]
        }
    }

    override operator fun minusAssign(other: Long): Unit {
        for (i in this.indices) {
            this.data[i] -= other
        }
    }

    operator fun timesAssign(other: MemoryViewLongArray): Unit {
        for (i in this.indices) {
            this.data[i] *= other.data[i]
        }
    }

    override operator fun timesAssign(other: Long): Unit {
        for (i in this.indices) {
            this.data[i] *= other
        }
    }

    operator fun divAssign(other: MemoryViewLongArray): Unit {
        for (i in this.indices) {
            this.data[i] /= other.data[i]
        }
    }

    override operator fun divAssign(other: Long): Unit {
        for (i in this.indices) {
            this.data[i] /= other
        }
    }
...
class MemoryViewFloatArray(override val data: FloatArray) : MemoryView<Float>() {
    ...
    operator fun plusAssign(other: MemoryViewFloatArray): Unit {
        for (i in this.indices) {
            this.data[i] += other.data[i]
        }
    }

    override operator fun plusAssign(other: Float): Unit {
        for (i in this.indices) {
            this.data[i] += other
        }
    }

    operator fun minusAssign(other: MemoryViewFloatArray): Unit {
        for (i in this.indices) {
            this.data[i] -= other.data[i]
        }
    }

    override operator fun minusAssign(other: Float): Unit {
        for (i in this.indices) {
            this.data[i] -= other
        }
    }

    operator fun timesAssign(other: MemoryViewFloatArray): Unit {
        for (i in this.indices) {
            this.data[i] *= other.data[i]
        }
    }

    override operator fun timesAssign(other: Float): Unit {
        for (i in this.indices) {
            this.data[i] *= other
        }
    }

    operator fun divAssign(other: MemoryViewFloatArray): Unit {
        for (i in this.indices) {
            this.data[i] /= other.data[i]
        }
    }

    override operator fun divAssign(other: Float): Unit {
        for (i in this.indices) {
            this.data[i] /= other
        }
    }
...
class MemoryViewDoubleArray(override val data: DoubleArray) : MemoryView<Double>() {
    ...
    override operator fun plusAssign(other: Double): Unit {
        for (i in this.indices) {
            this.data[i] += other
        }
    }

    operator fun minusAssign(other: MemoryViewDoubleArray): Unit {
        for (i in this.indices) {
            this.data[i] -= other.data[i]
        }
    }

    override operator fun minusAssign(other: Double): Unit {
        for (i in this.indices) {
            this.data[i] -= other
        }
    }

    operator fun timesAssign(other: MemoryViewDoubleArray): Unit {
        for (i in this.indices) {
            this.data[i] *= other.data[i]
        }
    }

    override operator fun timesAssign(other: Double): Unit {
        for (i in this.indices) {
            this.data[i] *= other
        }
    }

    operator fun divAssign(other: MemoryViewDoubleArray): Unit {
        for (i in this.indices) {
            this.data[i] /= other.data[i]
        }
    }

    override operator fun divAssign(other: Double): Unit {
        for (i in this.indices) {
            this.data[i] /= other
        }
    }
