...
class NDArray<T : Number, D : Dimension> constructor(
    data: ImmutableMemoryView<T>,
    override val offset: Int = 0,
    override val shape: IntArray,
    override val strides: IntArray = computeStrides(shape),
    ...
    override val consistent: Boolean
        get() {
...
//            if (dim.d != 1) throw IllegalStateException("NDArray of dimension ${dim.d}, use multiIndex.")
    ...
    override fun reshape(
        ...
        require(newShape.fold(1, Int::times) == size) {
            "Cannot reshape array of size $size into a new shape ${
                newShape.joinToString(
                    prefix = "(",
                    postfix = ")"
                )
            }"
