 ...
 */
operator fun <T : Number, D : Dimension> MultiArray<T, D>.plus(other: MultiArray<T, D>): NDArray<T, D> {
...
operator fun <T : Number, D : Dimension> MultiArray<T, D>.plus(other: T): NDArray<T, D> {
    val ret = if (this.consistent) (this as NDArray).clone() else (this as NDArray).deepCopy()
    ret += other
    return ret
}
 ...
 */
operator fun <T : Number, D : Dimension> MutableMultiArray<T, D>.plusAssign(other: MultiArray<T, D>): Unit {
    requireArraySizes(this.size, other.size)
    when {
...
operator fun <T : Number, D : Dimension> MutableMultiArray<T, D>.plusAssign(other: T): Unit {
    when {
        this.consistent -> {
            this.data += other
        }
 ...
 */
operator fun <T : Number, D : Dimension> MultiArray<T, D>.minus(other: MultiArray<T, D>): NDArray<T, D> {
    requireArraySizes(this.size, other.size)
    val ret = if (this.consistent) (this as NDArray).clone() else (this as NDArray).deepCopy()
    ret -= other
    return ret
...
operator fun <T : Number, D : Dimension> MultiArray<T, D>.minus(other: T): NDArray<T, D> {
    val ret = if (this.consistent) (this as NDArray).clone() else (this as NDArray).deepCopy()
    ret -= other
    return ret
}
 ...
 */
operator fun <T : Number, D : Dimension> MutableMultiArray<T, D>.minusAssign(other: MultiArray<T, D>): Unit {
    requireArraySizes(this.size, other.size)
    when {
        this.consistent && other.consistent -> {
            this.data -= (other.data as MemoryView)
 ...
 */
@Suppress("UNCHECKED_CAST")
operator fun <T : Number, D : Dimension> MutableMultiArray<T, D>.minusAssign(other: T): Unit {
    when {
        this.consistent -> {
            this.data -= other
        }
 ...
 */
operator fun <T : Number, D : Dimension> MultiArray<T, D>.times(other: MultiArray<T, D>): NDArray<T, D> {
    requireArraySizes(this.size, other.size)
    val ret = if (this.consistent) (this as NDArray).clone() else (this as NDArray).deepCopy()
    ret *= other
    return ret
...
operator fun <T : Number, D : Dimension> MultiArray<T, D>.times(other: T): NDArray<T, D> {
    val ret = if (this.consistent) (this as NDArray).clone() else (this as NDArray).deepCopy()
    ret *= other
    return ret
}
 ...
 */
operator fun <T : Number, D : Dimension> MutableMultiArray<T, D>.timesAssign(other: MultiArray<T, D>): Unit {
    requireArraySizes(this.size, other.size)
    when {
        this.consistent && other.consistent -> {
            this.data *= (other.data as MemoryView)
 ...
 */
@Suppress("UNCHECKED_CAST")
operator fun <T : Number, D : Dimension> MutableMultiArray<T, D>.timesAssign(other: T): Unit {
    when {
        this.consistent -> {
            this.data *= other
        }
 ...
 */
operator fun <T : Number, D : Dimension> MultiArray<T, D>.div(other: MultiArray<T, D>): NDArray<T, D> {
    requireArraySizes(this.size, other.size)
    val ret = if (this.consistent) (this as NDArray).clone() else (this as NDArray).deepCopy()
    ret /= other
    return ret
...
operator fun <T : Number, D : Dimension> MultiArray<T, D>.div(other: T): NDArray<T, D> {
    val ret = if (this.consistent) (this as NDArray).clone() else (this as NDArray).deepCopy()
    ret /= other
    return ret
}
 ...
 */
operator fun <T : Number, D : Dimension> MutableMultiArray<T, D>.divAssign(other: MultiArray<T, D>): Unit {
    requireArraySizes(this.size, other.size)
    when {
        this.consistent && other.consistent -> {
            this.data /= (other.data as MemoryView)
 ...
 */
@Suppress("UNCHECKED_CAST")
operator fun <T : Number, D : Dimension> MutableMultiArray<T, D>.divAssign(other: T): Unit {
    when {
        this.consistent -> {
            this.data /= other
        }
